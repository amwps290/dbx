...
export default withEnglishFallback({
  ...
  connection: {
    ...
    mqttReceived: "接收",
    mqttRetained: "保留",
    mqttUnsubscribeMissing: "找不到可取消的訂閱。Topic 清單可能已重新整理，請再試一次。",
    mqttUnsubscribeConfirm: "確定要取消訂閱「{topic}」嗎？",
    mqttUnsubscribeGroupConfirm: "確定要取消此群組下的 {count} 個訂閱嗎？",
    mqttUnsubscribeFailed: "有 {count} 個訂閱無法取消：{errors}",
    mqttUnsubscribeTopic: "取消訂閱 {topic}",
    mqttUnsubscribeGroup: "取消此群組下的 {count} 個訂閱",
    mqttTopic: "Topic",
  ...
  ai: {
    ...
    maxRetriesGlobalDescription: "遇到暫時性 API 錯誤（限流、超時、網路波動）時的自動重試次數。適用於所有 API 模式 AI 供應商。CLI 供應商（claude-code、codex、pi）不受影響。",
  ...
  mqMessages: {
    ...
    kafkaLatestHint: "從各分區末尾附近平均讀取，依時間倒序合併。",
    kafkaEarliestHint: "從每個選定分區最早可讀的訊息開始正序讀取。",
    kafkaOffsetHint: "從指定 Offset 開始正序讀取；填寫分區時只讀該分區，留空則對全部分區分別讀取。",
